  ...
  return (
    <div>
      <div className="flex items-end justify-between">
        <div>
          <div className="text-2xl font-semibold">Shipments</div>
          <div className="mt-1 text-sm text-white/60">Kanban pipeline for operational tracking</div>
        </div>
        <button
          onClick={refresh}
              ...
              <div className="p-3 space-y-3">
                {items.map((s) => (
                  <div key={s.id} className="rounded-xl border border-white/10 bg-secondary-900/40 p-3">
                    ...
                    <div className="mt-1 text-xs text-white/60">{s.estimated_delivery ? `ETA: ${s.estimated_delivery}` : 'ETA: —'}</div>
                  </div>
                ))}
